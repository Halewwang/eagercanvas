...
test('Seedance adapter creates Volcengine video tasks with normalized content payload', async () => {
  ...
  assert.equal(requests[0].body?.duration, 11)
...
test('Seedance adapter polls Volcengine task status and extracts content video urls', async () => {
  const originalFetch = global.fetch
  const requests = []
  const adapter = new SeedanceProviderAdapter()
  ...
  try {
    const result = await adapter.pollTaskStatus('cgt-20260403112523-rt66c', { apiKey: 'sk-test' })
