...
Eager Canvas 是一个面向 AI 图像、视频与 3D 内容生产的可视化工作台。项目提供无限画布、节点编排、项目管理、共享模板、用量统计，以及两套后台能力，前后端代码都已包含在这个仓库里。
...
| 节点类型 | Text、Image、Video、3D Model 四类主节点，支持提示词输入、图片预览、视频结果、3D 模型预览 |
| 图像能力 | 文生图、图生图、图片上传、远程素材入库、图片去背 |
| 视频能力 | 图像驱动视频生成，支持轮询任务状态并回填结果 |
| 3D 能力 | 基于提示词和多视角图片生成 3D 模型，并在画布内直接预览 |
...
| 3D | Hunyuan 3D Pro 3.1、Hunyuan 3D Pro 3.0 |
...
- 图像生成、视频生成、3D 生成、共享模板、用量统计、后台管理、真实账号体系都依赖后端与 Supabase。
