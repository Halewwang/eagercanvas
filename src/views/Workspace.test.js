...
test('workspace inbox aggregates invites and project edit requests in the sidebar', () => {
  const sidebarSource = readWorkspaceComponentSource('WorkspaceSidebar')
  const inboxSource = readWorkspaceComponentSource('WorkspaceInboxPanel')

  assert.match(workspaceApiSource, /export const apiGetWorkspaceInbox/)
  assert.match(workspaceApiSource, /url:\s*'\/workspace\/inbox'/)
  assert.match(projectsStoreSource, /apiReviewProjectEditRequest/)
  assert.match(workspaceSource, /import WorkspaceInboxPanel from '@\/components\/workspace\/WorkspaceInboxPanel\.vue'/)
  assert.match(workspaceSource, /workspaceInbox/)
  assert.match(workspaceSource, /loadWorkspaceInbox/)
  assert.match(workspaceSource, /reviewProjectEditRequest/)
  assert.match(workspaceSource, /const workspaceInboxCount = computed/)
  assert.match(workspaceSource, /\{ key: 'inbox', label: 'Inbox', icon: WindowSidebarOutline, count: workspaceInboxCount\.value \}/)
  assert.match(workspaceSource, /activeSection === 'inbox'/)
  assert.match(workspaceSource, /<WorkspaceInboxPanel[\s\S]*:workspace-invites="workspaceInbox\.workspaceInvites"/)
  assert.match(workspaceSource, /@accept-workspace-invite="handleAcceptWorkspaceInvite"/)
  assert.match(workspaceSource, /@review-project-request="handleReviewProjectRequest"/)
  assert.match(workspaceSource, /loadWorkspaceInbox\(\)/)

  assert.match(sidebarSource, /item\.count/)
  assert.match(sidebarSource, /class="nav-count"/)
  assert.match(sidebarSource, /\{\{ item\.count \}\}/)

  assert.match(inboxSource, /workspaceInvites/)
  assert.match(inboxSource, /projectEditRequests/)
  assert.match(inboxSource, /emit\('acceptWorkspaceInvite', invite\.id\)/)
  assert.match(inboxSource, /emit\('reviewProjectRequest', \{ projectId: request\.projectId, requestId: request\.id, decision \}\)/)
  assert.match(inboxSource, />Approve<\/button>/)
  assert.match(inboxSource, />Reject<\/button>/)
})

test('workspace owner settings expose workspace rename and project permission management', () => {
  const settingsSource = readWorkspaceComponentSource('WorkspaceSettingsPanel')

  assert.match(projectsApiSource, /export const apiGetProjectPermissions/)
  assert.match(projectsApiSource, /\/projects\/\$\{encodeURIComponent\(id\)\}\/permissions/)
  assert.match(projectsApiSource, /export const apiUpdateProjectPermission/)
  assert.match(projectsApiSource, /\/projects\/\$\{encodeURIComponent\(id\)\}\/permissions\/\$\{encodeURIComponent\(userId\)\}/)
  assert.match(projectsStoreSource, /apiGetProjectPermissions/)
  assert.match(projectsStoreSource, /apiUpdateProjectPermission/)
  assert.match(projectsStoreSource, /export const loadProjectPermissions = async \(id\) =>/)
  assert.match(projectsStoreSource, /export const updateProjectPermission = async \(id, userId, role\) =>/)

  assert.match(workspaceSource, /import WorkspaceSettingsPanel from '@\/components\/workspace\/WorkspaceSettingsPanel\.vue'/)
  assert.match(workspaceSource, /loadProjectPermissions/)
  assert.match(workspaceSource, /updateProjectPermission/)
  assert.match(workspaceSource, /const isTeamWorkspaceOwner = computed/)
  assert.match(workspaceSource, /currentWorkspace\.value\?\.kind === 'team' && currentWorkspace\.value\?\.role === 'owner'/)
  assert.match(workspaceSource, /isTeamWorkspaceOwner\.value\s*\?\s*\[\{ key: 'settings', label: 'Settings', icon: SettingsOutline \}\]/)
  assert.match(workspaceSource, /activeSection === 'settings'/)
  assert.match(workspaceSource, /<WorkspaceSettingsPanel[\s\S]*:current-workspace="currentWorkspace"/)
  assert.match(workspaceSource, /:permission-detail="projectPermissionDetail"/)
  assert.match(workspaceSource, /@update-workspace="saveWorkspaceSettingsFromPanel"/)
  assert.match(workspaceSource, /@load-project-permissions="loadProjectPermissionDetail"/)
  assert.match(workspaceSource, /@change-project-permission="changeProjectPermission"/)

  assert.match(settingsSource, /currentWorkspace/)
  assert.match(settingsSource, /permissionDetail/)
  assert.match(settingsSource, /emit\('updateWorkspace'\)/)
  assert.match(settingsSource, /emit\('loadProjectPermissions', project\.id\)/)
  assert.match(settingsSource, /emit\('changeProjectPermission', \{ projectId, userId, role \}\)/)
  assert.match(settingsSource, /option value="owner">Owner<\/option>/)
  assert.match(settingsSource, /option value="editor">Editor<\/option>/)
  assert.match(settingsSource, /option value="viewer">Viewer<\/option>/)
})
