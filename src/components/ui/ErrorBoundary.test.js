...
test('root app wraps router content with global stability UI modules', () => {
  ...
  assert.match(appSource, /import \{ computed, onBeforeUnmount, onMounted \} from 'vue'/)
  assert.match(appSource, /import \{ useRoute \} from 'vue-router'/)
  assert.match(appSource, /const route = useRoute\(\)/)
  assert.match(appSource, /const appRouteClass = computed\(\(\) =>/)
  assert.match(appSource, /route\.name === 'Home'/)
  assert.match(appSource, /'app-route--home'/)
  assert.match(appSource, /'app-route--google-sans'/)
  ...
  assert.match(appSource, /<ErrorBoundary>/)
  assert.match(appSource, /<n-config-provider :theme="darkTheme" :theme-overrides="themeOverrides" :class="appRouteClass">/)
