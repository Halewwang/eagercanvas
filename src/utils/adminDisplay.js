...
export const getAdminOverviewCards = ({
  ...
  return [
    ...
    {
      label: useOfficialUsage ? '服务商调用量' : '总调用量',
      value: canReadUsage ? usageCalls || 0 : '--',
      note: canReadUsage ? (useOfficialUsage ? '来自用户服务商调用数据' : '全局请求总次数') : '无用量权限'
    },
    {
      label: useOfficialUsage ? `服务商消耗 (${currency})` : `官方消耗 (${currency})`,
      value: canReadUsage ? formatAdminUsd(costAmount) : '--',
      note: canReadUsage ? (useOfficialUsage ? '来自服务商 usage-log' : '累计官方消耗') : '无用量权限'
    },
